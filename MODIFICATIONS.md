...
- `lmcache/v1/gpu_connector/gpu_connectors.py` - optional Putpocket exact-load position-remap bypass and source-to-target RoPE remap for layerwise GPU transfer
- `lmcache/v1/compute/attention/utils.py` - lazy sparse-backend import for CacheBlend recompute
- `lmcache/v1/compute/attention/flash_attn.py` - vLLM Attention import compatibility for CacheBlend recompute
- `lmcache/v1/compute/attention/flash_infer_sparse.py` - vLLM Attention import compatibility for sparse CacheBlend recompute

**Current state**: The vLLM v1 adapter recognizes an optional `putpocket_blending_mode` value in LMCache `extra_config`; `special` now means the Putpocket hook is available, not that every request must use it. A request enters the Putpocket path only when `SamplingParams.extra_args["kv_transfer_params"]` carries `putpocket.enable=True` with valid Putpocket metadata. Requests without valid metadata use normal LMCache lookup/retrieval behavior; special mode avoids eager CacheBlend construction because this vLLM worker path has not registered the model when the connector is initialized. Scheduler lookup normalizes vLLM token containers to plain Python lists before RPC serialization. The adapter passes vLLM tensor/pipeline parallel sizes and the vLLM runtime config into the Putpocket executor so unsupported TP layouts can fail explicitly and temp materializers can resolve the exact runtime RoPE/YaRN params before any KV tensor update is attempted; it can now instantiate a named Putpocket executor from `extra_config["putpocket_executor"]`. Putpocket plans may provide `lmcache_hit_token_len`, allowing query-boundary planners to reserve less than the full prompt as externally supplied KV; requests with `putpocket.profile_only=True` build/log the plan but fall back to normal LMCache lookup so experiments can collect planner data. Layerwise store/retrieve calls now forward request configs into `LMCacheEngine.store_layer` and `retrieve_layer` so tagged namespaces affect LMCache keys consistently with the non-layerwise path; worker-side layerwise retrieve also forwards `req_id` so retrieve cleanup can distinguish request-level lookup pins from retrieve-owned staging pins. Layerwise store generator advancement is StopIteration-safe so partial-chunk storage does not crash vLLM workers; if storage completed early, any remaining issue surfaces as a later LMCache miss. Layerwise retrieve cleanup now preserves LocalCPU exact-hit objects whose pins are owned by `lookup_unpin(req_id)`, while still unpinning retrieve-owned staging objects such as disk-loaded CPU buffers. For Putpocket materialization requests, the scheduler derives a sanitized LMCache key namespace from Putpocket controls, using only the fixed `lmcache.tag.putpocket_fixing` value for exact lookup/retrieve/store while keeping `putpocket.*` as control metadata for the worker hook. On a target exact hit, normal LMCache retrieve handles the fixed namespace; on miss, the adapter attaches a Putpocket source-prefix plan and passes both prefix and full request token/slot views into the worker hook so stage-2 can use scheduled query-tail slots as scratch space without marking them as cached; the worker also exposes the active vLLM config on the LMCache engine for Putpocket's worker-side CacheBlend construction. Putpocket exact-load requests can mark layerwise GPU transfer with `putpocket_disable_position_remap=True`; this bypasses the CacheBlend fused-RoPE dependency because Putpocket stores fixed/full KV under the target token positions already. Putpocket source materialization can also pass source-to-target reusable spans to the layerwise GPU connector, which compacts the source KV spans, applies old-position to new-position K correction with the runtime vLLM rotary cache, and writes the corrected K/V into the target request slots. CacheBlend attention backends import vLLM's `Attention` class from the current model-executor layer path used by this fork, so Putpocket dirty-token recompute can construct its blender under vLLM v1. The optional FlashInfer sparse backend is imported lazily only when that backend is selected, avoiding an import-time dependency on FlashInfer symbols that are absent from the current environment while the active FlashAttention path is used.
...
- 2026-05-25 - refined: derived sanitized Putpocket fixed-cache LMCache configs from control metadata and passed full request token/slot views to the worker hook for query-tail scratch probing.
- 2026-05-25 - refined: added Putpocket source-to-target layerwise remap support so insert/delete shifted reusable spans can be copied into target slots with runtime-vLLM RoPE correction.
- 2026-05-26 - refined: updated CacheBlend attention backend imports to the current vLLM `model_executor.layers.attention` path so Putpocket materialization can instantiate the blender.
- 2026-05-26 - refined: made the optional FlashInfer sparse backend import lazy so FlashAttention-based Putpocket recompute does not fail on missing FlashInfer sparse helpers.
- 2026-05-26 - refined: exposed the active vLLM config on the worker LMCache engine so Putpocket can construct CacheBlend under vLLM's config context.
